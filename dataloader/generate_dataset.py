...
# --- Amplified noise version ---
def synthesize_amplified_noise_audio(
    sample_rate=16000,
    duration=60.0,
    burst_len_range=(0.5, 0.65),
    burst_amp_range=(10.0, 20.0),
    burst_freq_hz=800,  # kept for compatibility but not used
    noise_std=0.01,
    have_signal=1,
    seed=42
):
    """
    Similar to synthesize_random_burst_audio, but the signal is generated by
    randomly selecting a segment of the initial noise and amplifying it by a random factor.
    
    Args:
        sample_rate (int): Sampling rate in Hz.
        duration (float): Total duration in seconds.
        burst_len_range (tuple): Duration range for the amplified segment (min, max) in seconds.
        burst_amp_range (tuple): Amplification factor range (min, max).
        burst_freq_hz (float): Kept for compatibility with generate_sparse_burst_dataset (not used).
        noise_std (float): Standard deviation of Gaussian noise.
        have_signal (int): If 0, returns pure noise; if 1, includes amplified segment.
        seed (int): Random seed for reproducibility.
    
    Returns:
        audio (torch.Tensor): Generated audio signal.
        burst_info (dict): Metadata about the amplified segment.
    """
    if seed is not None:
        torch.manual_seed(seed)
        np.random.seed(seed)
    ...
    # Insert amplified noise segment
    start_idx = int(burst_start_sec * sample_rate)
    end_idx = start_idx + int(burst_duration_sec * sample_rate)
    
    # Create a copy of the audio with the amplified segment
    audio = noise.clone()
    if end_idx > start_idx:
        # Amplify the selected noise segment by the burst_amp factor
        audio[start_idx:end_idx] = noise[start_idx:end_idx] * burst_amp
